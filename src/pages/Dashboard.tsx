...
const Dashboard = () => {
    ...
    const updateUserRoleMutation = useMutation({
        ...
        onSuccess: (updatedProfile) => {
            ...
            toast({ title: "Role Updated", description: `${updatedProfile.name || updatedProfile.email} is now a(n) ${updatedProfile.role}.` });
        },
        onError: (error) => {
                                            ...
                                            {foundUser ? (
                                                <div>
                                                    ...
                                                    <div className="mt-2 flex gap-2">
                                                        <Button size="sm" onClick={() => updateUserRoleMutation.mutate({ userId: foundUser.id, newRole: 'admin' })} disabled={foundUser.role === 'admin' || updateUserRoleMutation.isPending} className="bg-green-600 hover:bg-green-700"><UserCheck className="h-4 w-4 mr-2" /> Make Admin</Button>
                                                        <Button size="sm" variant="destructive" onClick={() => updateUserRoleMutation.mutate({ userId: foundUser.id, newRole: 'customer' })} disabled={foundUser.role !== 'admin' || updateUserRoleMutation.isPending}><UserX className="h-4 w-4 mr-2" /> Make Customer</Button>
                                                    </div>
                                                </div>
